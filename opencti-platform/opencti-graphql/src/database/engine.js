...
export const elMarkElementsAsDraftDelete = async (context, user, elements) => {
  ...
  const { relations, relationsToRemoveMap } = await getRelationsToRemove(context, SYSTEM_USER, elements, { includeDeletedElements: true });
  ...
  // 04/ Remove draft_ids from live relations and live elements of draft reverts
